      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-slate-500 text-xs mt-1">Across {stats?.totalWeeks || 0} weeks</p>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-3xl font-bold text-white">{stats?.totalUniqueIPs?.toLocaleString() || 0}</p>
            <p className="text-slate-500 text-xs mt-1">All-time unique visitors</p>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-3xl font-bold text-white">{stats?.averagePerWeek?.toLocaleString() || 0}</p>
            <p className="text-slate-500 text-xs mt-1">{stats?.averageUniqueIPsPerWeek || 0} unique IPs/week</p>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-3xl font-bold text-white">{currentWeek?.visits?.toLocaleString() || 0}</p>
            <p className="text-slate-500 text-xs mt-1">{currentWeek?.uniqueIPs || 0} unique IPs</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-gradient-to-br from-green-900/20 to-green-800/10 backdrop-blur-sm border border-green-700/50 rounded-xl p-6 shadow-xl">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="text-lg font-semibold text-green-300">Best Week</h3>
              </div>
              <div className="flex items-baseline gap-2">
                <p className="text-4xl font-bold text-green-400">{stats.highestWeek.visits.toLocaleString()}</p>
                <p className="text-slate-400">visits</p>
              </div>
              <p className="text-slate-400 text-sm mt-2">
                {formatWeekLabel(stats.highestWeek.week)} • {stats.highestWeek.uniqueIPs} unique IPs
              </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-orange-900/20 to-orange-800/10 backdrop-blur-sm border border-orange-700/50 rounded-xl p-6 shadow-xl">
              ...
              <div className="flex items-baseline gap-2">
                <p className="text-4xl font-bold text-orange-400">{stats.lowestWeek.visits.toLocaleString()}</p>
                <p className="text-slate-400">visits</p>
              </div>
              <p className="text-slate-400 text-sm mt-2">
                {formatWeekLabel(stats.lowestWeek.week)} • {stats.lowestWeek.uniqueIPs} unique IPs
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg transition-colors ${viewType === "bar" ? "bg-blue-500 text-white" : "bg-slate-700 text-slate-300 hover:bg-slate-600"}`}
                >
                  Bar
                </button>
                <button
                  onClick={() => setViewType("line")}
                  className={`px-4 py-2 rounded-lg transition-colors ${viewType === "line" ? "bg-blue-500 text-white" : "bg-slate-700 text-slate-300 hover:bg-slate-600"}`}
                >
                  Line
                </button>
              </div>
            ...
            <ResponsiveContainer width="100%" height={400}>
              {viewType === "bar" ? (
                <BarChart data={visits} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                  ...
                  <XAxis dataKey="week" angle={-45} textAnchor="end" height={80} tick={{ fill: "#94a3b8", fontSize: 12 }} />
                ...
                <LineChart data={visits} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                  ...
                  <Tooltip
                    ...
                    labelFormatter={formatWeekLabel}
                  />
                  <Line type="monotone" dataKey="visits" stroke="#3b82f6" strokeWidth={3} dot={{ fill: "#3b82f6", r: 5 }} activeDot={{ r: 7 }} />
                </LineChart>
              )}
            </ResponsiveContainer>
          ) : (
            <div className="h-96 flex items-center justify-center text-slate-400">No data available</div>
          )}
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-right py-3 px-4 text-slate-300 font-medium">Unique IPs</th>
                  ...
                  return (
                    <React.Fragment key={item.week}>
                      <tr className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="py-3 px-4 text-right text-blue-400 font-semibold">{item.visits.toLocaleString()}</td>
                        <td className="py-3 px-4 text-right text-emerald-400 font-semibold">{ipAddresses.length}</td>
                            ...
                            <div className="pl-4">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                                {ipAddresses.map((ip, idx) => (
                                  <div key={idx} className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm font-mono text-slate-300">
                                    {ip}
                                  </div>
                                ))}
                              </div>
